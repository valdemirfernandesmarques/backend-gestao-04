...
exports.criarVenda = async (req, res) => {
  ...
  try {
    const { alunoId, itensVenda, descontos = 0, metodoPagamento = 'PIX', escolaId: escolaIdBody, usuarioId } = req.body;
    ...
    const produtosIds = itensVenda.map(i => i.produtoId);
    const produtos = await db.Produto.findAll({ 
      where: { id: produtosIds, escolaId: escolaIdFinal }, 
      transaction: t 
    });

    if (produtos.length !== itensVenda.length) {
      await t.rollback();
      return res.status(400).json({ error: 'Um ou mais produtos não foram encontrados nesta escola.' });
    }
...
// Apagar venda e devolver o stock
